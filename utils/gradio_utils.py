...
def save_mask_as_image(im_mask, size, resolution, output_dir="masks"):
    import os
    os.makedirs(output_dir, exist_ok=True)
    # 将张量转为 NumPy 数组

    mask_np = im_mask.cpu().numpy() if torch.is_tensor(im_mask) else im_mask
    # 遍历每张掩码
    for i, mask in enumerate(mask_np): # 枚举第一维
        for j, mask2 in enumerate(mask): # 枚举第二维
            # 重塑掩码为 2D 分辨率
            mask_2d = mask2.reshape(resolution).astype(np.uint8) * 255  # 转为 0/255 以适配图片格式
            # 转为 PIL 图片
            mask_image = Image.fromarray(mask_2d, mode="L")  # "L" 表示灰度图
            # 保存图片
            mask_image.save(f"{output_dir}/matrix_mask{size}_{i + 1}_{j + 1}.png")
            print(f"Saved mask {i + 1} to {output_dir}/matrix_mask{size}_{i + 1}_{j + 1}.png")

def cal_attn_mask_xl(total_length,id_length,sa32,sa64,height,width,device="cuda",dtype= torch.float16):
    nums_1024 = (height // 32) * (width // 32)  # 每层 patch 为 32 * 32， 有 576 个 patchs / tokens
    ...
    #save_mask_as_image(bool_matrix1024.unsqueeze(1).reshape(total_length, total_length, nums_1024), 1024, resolution=((height // 32), (width // 32)))
    #save_mask_as_image(bool_matrix4096.unsqueeze(1).reshape(total_length, total_length, nums_4096), 4096, resolution=((height // 16), (width // 16)))
    ...
    # [Batch_Size, total_length * nums_1024, total_length * nums_1024]
...
class AttnProcessor(nn.Module):  # 原始自注意力代码
    r"""
...
class AttnProcessor2_0(torch.nn.Module): # 支持更新版本的pytorch
    r"""
    ...
    def __call__(
        self,
        attn,
        hidden_states, # 传入的隐藏状态（通常是图像特征 x）
        encoder_hidden_states=None, # 编码器的隐藏状态（如果有交叉注意力的话）
        attention_mask=None,
        temb=None,
    ):
        residual = hidden_states
        ...
        if encoder_hidden_states is None: # 空的话就复制一份
            encoder_hidden_states = hidden_states
        elif attn.norm_cross:
            encoder_hidden_states = attn.norm_encoder_hidden_states(encoder_hidden_states) # 获得交叉注意力隐藏层
        ...
        # the output of sdp = (batch, num_heads, seq_len, head_dim)  先换位置并行算多头
        # TODO: add support for attn.scale when we move to Torch 2.1
        hidden_states = F.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )
